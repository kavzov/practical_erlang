...
splitwith(Pred, [V | Rest], Acc) ->
    case Pred(V) of
        false -> {lists:reverse(Acc), [V | Rest]};
        true -> splitwith(Pred, Rest, [V | Acc])
    end.
...
zipwith(_Pred, [], _, Acc) -> lists:reverse(Acc);
zipwith(_Pred, _, [], Acc) -> lists:reverse(Acc);
zipwith(Pred, [V1 | Rest1], [V2 | Rest2], Acc) ->
    zipwith(Pred, Rest1, Rest2, [Pred(V1, V2) | Acc]).
